...
You can register http://asciidoctor.org/docs/asciidoctorj/#extension-api[AsciidoctorJ extensions] with nested extensions elements, composed of two attributes :

* `blockName` : the name of the block concerned by extension
* `className` : the full qualified name of class implementing extension
...
...
    <asciidoctor:convert sourceDirectory="src/asciidoc" outputDirectory="target" backend="html5">
        <inlineMacro blockName="twitter" className="org.asciidoctor.ant.extensions.TwitterMacro"/>
   </asciidoctor>
...
----
